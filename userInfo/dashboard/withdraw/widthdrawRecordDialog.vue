<template>
  <el-dialog
    ...
    :title="$t('message.home.withdrawRecord' /**提现记录 */)"
    :center="true"
      ...
      <div v-if="staticListData.length">
        <div class="record" v-for="item in tableListData" :key="item.order_no">
          <div class="record-item flex-between">
            <div class="label">
              {{ $t("message.home.serialNumber" /** 订单号*/) }}：
            </div>
            <div class="label">
              {{ item.order_no }}
          ...
          <div class="record-item flex-between">
            <div class="label">{{ $t("message.home.time" /**时间 */) }}：</div>
            <div class="label">{{ item.createTime }}</div>
          </div>
          <div class="record-item flex-between">
            <div class="label">
              {{ $t("message.home.amount" /** 金額*/) }}：
            </div>
            <div class="value">{{ item.amount }}</div>
          </div>
          <div class="record-item flex-between">
            <div class="label">
              {{ $t("message.home.orderStatus" /**状态 */) }}：
            </div>
